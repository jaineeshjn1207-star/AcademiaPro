...
from .similarity import compute_coding_problem_similarity
...
def student_can_view_leaderboard(user, exam):
    """A student may view this exam's leaderboard if they were ever part of
    its cohort — either granted access (StudentExamAccess) or an attempt
    record (StudentExamSession) — regardless of whether their access was
    later cleared after submission."""
    return (StudentExamAccess.objects.filter(exam=exam, student=user).exists()
            or StudentExamSession.objects.filter(exam=exam, student=user).exists())
...
class LeaderboardView(views.APIView):
    ...
    def get(self, request, exam_id):
        ...
        if is_faculty:
            if not faculty_can_view_exam(request.user, exam):
                return Response({'error': 'This assessment is not available.'}, status=status.HTTP_403_FORBIDDEN)
        elif not student_can_view_leaderboard(request.user, exam):
            return Response({'error': 'This assessment is not available.'}, status=status.HTTP_403_FORBIDDEN)
...
class ResultPdfView(views.APIView):
    ...
    def get(self, request, exam_id):
        session = StudentExamSession.objects.filter(exam_id=exam_id, student=request.user).select_related('exam', 'student', 'faculty_verified_by').first()
        ...
        from portal.pdf_generator import generate_exam_result_pdf
        stream = generate_exam_result_pdf(session)
...
class CodeSimilarityView(views.APIView):
    """
    Pairwise code similarity and academic integrity analysis for a coding problem.
    Accessible only to authorized faculty and admin users.
    """
    permission_classes = [IsFaculty]

    def get(self, request, exam_id, problem_id):
        try:
            exam = Exam.objects.get(id=exam_id)
        except Exam.DoesNotExist:
            return Response({'error': 'Exam not found'}, status=status.HTTP_404_NOT_FOUND)
        ...
        try:
            problem = CodingProblem.objects.get(id=problem_id, exam=exam)
        except CodingProblem.DoesNotExist:
            return Response({'error': 'Coding problem not found'}, status=status.HTTP_404_NOT_FOUND)

        raw_threshold = request.query_params.get('threshold', '0.70')
        try:
        ...
        result = compute_coding_problem_similarity(exam, problem, threshold=threshold)
        audit(request, 'code_similarity_checked', problem, {
            'exam_id': exam.id,
            'threshold': threshold,
            'compared_submissions': result.get('compared_submissions', 0),
            'flagged_pairs_count': len(result.get('flagged_pairs', [])),
        })
